...
rune_factory_4_location = {}
rune_factory_4_location.__index = rune_factory_4_location
...
---@type table<string, rune_factory_4_new_return>
NAMED_LOCATIONS = {}
local stale = true
...
---@class rune_factory_4_new_return
...
---@field name string
---@field side string?
---@field exits table<integer, {[1]:rune_factory_4_new_return, [2]:fun(): accessibilityLevel}>
---@field keys integer
...
---@param name string
---@return rune_factory_4_new_return
function rune_factory_4_location.new(name)
	if name == nil then
		error("rune_factory_4_location name cannot be nil")
	end
	if NAMED_LOCATIONS[name] then
		print("rune_factory_4_location " .. name .. " already exists")
	end
    local self = setmetatable({}, rune_factory_4_location)
	self.name = name
...
---@param exit string|rune_factory_4_new_return rune_factory_4_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function rune_factory_4_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = rune_factory_4_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
---@param exit string|rune_factory_4_new_return rune_factory_4_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function rune_factory_4_location:connect_two_ways(exit, rule)
...
---@param exit string|rune_factory_4_new_return rune_factory_4_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function rune_factory_4_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
---@param exit string|rune_factory_4_new_return rune_factory_4_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function rune_factory_4_location:connect_two_ways_entrance(name, exit, rule)
...
function rune_factory_4_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
---@param rule1? fun(): accessibilityLevel|boolean
---@param rule2? fun(): accessibilityLevel|boolean
function rune_factory_4_location:connect_two_ways_stuck(exit, rule1, rule2)
...
---@return accessibilityLevel
function rune_factory_4_location:accessibility()
...
---@param keys integer
function rune_factory_4_location:discover(accessibility, keys)
