...
const REPO='/mnt/c/Users/Russell/source/repos/rune4-poptracker'
const cats=JSON.parse(fs.readFileSync(process.argv[2],'utf8'))
...
const cache=new Map()
async function img(p){ if(!p) return null
...
function textTile(x,ox,oy,label){
  ...
  x.strokeStyle='rgba(255,255,255,.18)'; x.lineWidth=1; x.strokeRect(ox+.5,oy+.5,TILE-1,TILE-1)
  x.fillStyle='#e8dcc0'; x.textAlign='center'; x.textBaseline='middle'
  let size,lines=null
  for(size=10;size>=6;size--){ x.font=size+'px DV'
    const w=label.split(/\s+/),out=[]; let cur='',ok=true
    for(const t of w){ const s=cur?cur+' '+t:t
...
async function build(mapName,file,items,monsters){
  const groups=new Map()
  for(const it of items){ const g=it.path.split('/')[0]
  ...
  const g=x.createLinearGradient(0,0,0,H); g.addColorStop(0,'#f7f0d6'); g.addColorStop(1,'#eadfb4')
  ...
  for(const [region,v] of groups){
    ...
    x.fillStyle='#4a3a12'; x.font='bold 12px DVB'; x.textAlign='right'; x.textBaseline='middle'
    x.fillText(region, GUT-12, y+rows*P/2-GAP)
    for(let i=0;i<v.length;i++){
      const ox=GUT+(i%COLS)*PW, oy=y+Math.floor(i/COLS)*P, ay=oy+BAND
      const src = monsters ? 'images/monsters/'+slug(v[i].name)+'.png' : v[i].img
...
const out=[]
for(const region of ['Forge','Crafting','Cooking']){
  const items=cats.Crafting.filter(i=>i.path.split('/')[0]===region)
  out.push(await build(region,`grid_${slug(region)}.png`,items,false))
}
fs.writeFileSync(outdir+'/grid_split.json', JSON.stringify(out))
for(const o of out) console.log(`${o.mapName.padEnd(12)} ${o.W}x${o.H}  ${o.pins.length} tiles`)
